...
export default function MagneticDock({
  ...
  return (
    <motion.div
      ...
      className={`flex h-16 items-end gap-4 rounded-2xl px-4 pb-3 border ${
        darkMode 
...
function IconContainer({ mouseX, distance, magnification, baseSize, item, darkMode }) {
  ...
  return (
    <motion.div
      ...
      className={`group relative aspect-square rounded-full flex items-center justify-center cursor-pointer transition-colors ${
    ...
    >
      ...
      {/* Tooltip */}
      <div className={`absolute -top-10 left-1/2 -translate-x-1/2 px-2 py-1 rounded text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap ${
